...
Kaza tarihi diğer bir ifadeyle maluliyet tarihi ile bilirkişi hesaplama tarihi arasındaki sürede malulün yıllar itibariyle gelir bilgisi, maluliyet oranı, şirket kusur oranı kullanılarak yapılan Bilinen Dönem tazminat tutarları aşağıdaki tabloda yıl bazında detaylı olarak verilmektedir.



```{r echo=FALSE}

# knitr::kable(Tam_Maluliyet_Donemi_Tablosu, caption = "Geçici Maluliyet Tablosu")
...
str_glue("Bilinen Dönem Maluliyet Tazminatı toplam {sirket_odeme_tarihi_bilinen_donem_toplam_tazminat} TL olarak hesaplanmıştır.")
...
knitr::kable(HT_Tam_Maluliyet_Donemi_Tablosu, caption = "Hesap Tarihi - Geçici İş Göremezlik Tablosu")


hesap_tarihi_gecici_maluliyet_tazminat <- formattable::currency(HT_gecici_maluliyet_tazminat, digits = 2L, format = "f", big.mark = ".",decimal.mark = ",",  symbol = "")


str_glue("Geçici İş Göremezlik Tazminatı toplam {hesap_tarihi_gecici_maluliyet_tazminat} TL olarak hesaplanmıştır.")                            


knitr::kable(HT_Maluliyet_Sonrasi_Donem_Tablosu, caption = "Hesap Tarihi - Sürekli Maluliyet Tablosu")


hesap_tarihi_surekli_maluliyet_tazminat <- formattable::currency(HT_surekli_maluliyet_tazminat, digits = 2L, format = "f", big.mark = ".",decimal.mark = ",",  symbol = "")


str_glue("Bilinen Dönem Maluliyet Tazminatı toplam {hesap_tarihi_surekli_maluliyet_tazminat} TL olarak hesaplanmıştır.")
...
if(Toplam_Tazminat >= kaza_tarihi_teminat_limiti) {
  str_glue("Ancak, kaza tarihi itibariyle trafik sigortası sakatlık teminatı {kaza_tarihi_teminat_limiti } TL’dir. Hesaplanan tazminatın teminat limitinin üstünde kaldığı görülmüştür. Bu itibarla, davalı şirketin sürekli maluliyet tazminatı teminat limiti olan {kaza_tarihi_teminat_limiti} TL’dir.") } else {str_glue(" ")} 
...
Yargıtay Kararları gereği bakıcı gideri hesaplaması ilgili dönem brüt asgari ücret üzerinden hesaplanmıştır.
...
bakiye_tazminat <- toplam_kismi_odeme <- formattable::currency(bakiye_tazminat, digits = 2, format = "f", big.mark = ".",decimal.mark = ",",  symbol = "")
...
knitr::kable(son_hesaplama_tablosu , caption = "Bakiye Tazminat Tablosu")
str_glue("•	Kaza tarihli trafik sigortası sakatlık teminat limiti kapsamında davalı şirket tarafından yapılan toplam ödemelerin düşülmesi sonucu hesaplanan toplam bakiye sürekli maluliyet tazminatı {bakiye_tazminat} TL hesaplanmıştır.")


knitr::kable(bakici_son_hesaplama_tablosu , caption = "Bakıcı Gideri Tazminat Tablosu")
str_glue("{Bakıcı_gideri_suresi} gün süreli Toplam Bakıcı Gideri tazminatı {Toplam_Bakici_Gideri} TL'dir.")
